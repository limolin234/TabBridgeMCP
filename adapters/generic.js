...
module.exports = {
  ...
  plan: (url, options = {}) => {
    ...
    if (mode === 'links') return { fields: [{ key: 'links', kind: 'list', selector: options.selector || 'a[href]', limit: 100, properties: ['text', 'href', 'tag'] }] };
    if (mode === 'controls') return { fields: [{ key: 'controls', kind: 'list', selector: options.selector || 'input, textarea, select, button', limit: 100, properties: ['tag', 'type', 'name', 'id', 'placeholder', 'text', 'disabled'] }] };
    if (mode === 'elements') return { fields: [{ key: 'elements', kind: 'list', selector: options.selector || 'body *', limit: 100, properties: ['tag', 'id', 'class', 'role', 'aria-label', 'name', 'type', 'placeholder', 'text', 'href', 'src', 'disabled'] }] };
