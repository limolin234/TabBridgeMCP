...
部分出版商用 HTML 包装页提供 PDF（如 IEEE 的 `stamp.jsp`）。当请求的 URL
是已知包装页时，server 会解析出真实文件 URL，先在标签页里加载该文件（预热
出版商的访问检查），再从这个已预热的环境中抓取。如果访问检查把导航弹回原
页面，下载会以明确的"需要机构登录"错误失败，而不是保存一个挑战页面。
